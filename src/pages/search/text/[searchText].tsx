...
import { getDocs, setDoc, getDoc, query, collection, orderBy, doc, deleteDoc, 
	updateDoc, limit, limitToLast, startAfter, startAt, endBefore, endAt, where } from "firebase/firestore";
import { whiteBtn } from 'src/styles/global';

const limitValue = 24;

// export async function getServerSideProps({ query: { searchText } }) {
...
const TextSearchResultPage = ({ searchText }) => {
	...
	const directionFetch = (direction) => {
		if(direction == 'next') {
			getDocs(query(collection(db, 'products'), orderBy('title'), startAt(searchText),
				endAt(searchText + '\uf8ff'), startAfter(lastDoc), limit(limitValue)))
			.then((snapshot) => {
			...
			getDocs(query(collection(db, 'products'), orderBy('title'), startAt(searchText),
				endAt(searchText + '\uf8ff'), endBefore(firstDoc), limitToLast(limitValue)))
			.then((snapshot) => {
				setFirstDoc(snapshot.docs[0])
				setLastDoc(snapshot.docs[snapshot.docs.length-1])
				const data = snapshot.docs.map(v => {
	...
	const fetchSearchData = () => {
		try {
			getDocs(query(collection(db, 'products'), orderBy('title'), startAt(searchText),
				endAt(searchText + '\uf8ff'), limit(limitValue)))
			.then((snapshot) => {
				setFirstDoc(snapshot.docs[0])
				setLastDoc(snapshot.docs[snapshot.docs.length-1])
				const data = snapshot.docs.map(v => {
	...
	return (
		<Box>
			<Grid container mb={5}>
				<Typography variant="h6" mt={2}>&quot;{searchText}&quot;에 대한 검색결과</Typography>
				...
				<Grid mt={2} container direction="row" justifyContent="start" alignItems="center">
					{products.length != 0 
						...
						: <Grid container pt={2} pb={2}>검색 결과가 없습니다. 다른 검색어를 입력해보세요</Grid>
					}
				</Grid>
...
TextSearchResultPage.getInitialProps = async ({ query }) => {
